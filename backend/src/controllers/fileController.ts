 ...
 */
const verifyFileAccess = async (fileId: string, req: Request): Promise<any> => {
  ...
  const meta = await FileMetadata.findOne({ fileId, status: 'active' });
  if (!meta) {
...
export const listFiles = async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      console.error('[listFiles] Google Drive list failed, falling back to local DB cache:', (driveError as Error).message);
      // Fallback: Query MongoDB FileMetadata!
      const dbFiles = await FileMetadata.find({ parentId: parentId, status: 'active' }).lean();
      files = dbFiles.map(f => ({
        id: f.fileId,
        name: f.name,
        mimeType: f.type,
...
export const bulkDownload = async (req: Request, res: Response) => {
  try {
    ...
    let fileIds: string[] | undefined =
      req.body?.fileIds ||
      (req.query.fileIds ? (req.query.fileIds as string).split(',') : undefined);
...
export const getAllUsers = async (req: Request, res: Response) => {
  try {
    const users = await User.find({ isEmailVerified: true }).select('-passwordHash').sort({ createdAt: -1 });
    res.json(users);
  } catch (error) {
    res.status(500).json({ message: (error as Error).message });
  }
    ...
    (async () => {
      try {
        ...
        if (status === 'approved') {
          ...
          `;
          await sendCustomEmail(user.email, 'DriveFlow: Update regarding your account registration', rejectedHtml);
        }
      } catch (err) {
